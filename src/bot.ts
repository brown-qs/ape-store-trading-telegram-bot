...
export class swapBot {
  ...
  listenCommand() {
    ...
    this.bot.onText(/\/bindChannel/, async (msg, match) => {
      if (msg.from.username == this.params.adminName) {
        ...
        this.bot.sendMessage(msg.chat.id, `You are not an administrator that cannot be set up`)
      }
    });
  ...
  async handleWatchAndFollow(res: callbackEvent) {
    ...
    console.log(`Belonging chain：${chainEnum[res.chainId]}，Block：${res.blockNumber}，Order：${res.transactions.length} Pen，Network speed ${res.ms} ms，${time}`)
      ...
      if (wallets.length) {
        ...
        if (find) {
          ...
          if (result) {
            ...
            user.log_id = 0
            this.updateUserSetting(user)
            this.bot.sendMessage(msg.chat.id, "Delete the linked wallet address successfully")
          } else {
            this.bot.sendMessage(msg.chat.id, "Delete the binding wallet address failed")
          }
        } else {
          this.bot.sendMessage(msg.chat.id, "The wallet address is not binding")
        }
      } else {
  ...
  async addRush(msg: Message) {
    ...
    if (find) {
      ...
      if (findTask) {
        ...
        user.log_id = 0
        this.updateUserSetting(user)
        this.bot.sendMessage(msg.chat.id, `The start of the contract with the existence of the contract`)
        return
      }
      let insert = {
  ...
  async rushDetail(query: any, id: Number) {
    ...
    if (contract) {
      ...
      this.bot.sendMessage(query.message.chat.id, "No contract information is found")
    }
  }
  ...
  async switchRouter(query: any) {
    switch (query.data) {
      ...
      //Delete wallet
      case "delete_wallet":
        this.bot.sendMessage(query.message.chat.id, `Please enter the wallet address to delete`
          , {
            "reply_markup": {
              "force_reply": true
            }
      ...
      //set upprc
      case "smart_money":
        console.log("smart_money")
        if (query.from.username == this.params.adminName) {
          this.bot.sendMessage(query.message.chat.id, `Please enter the contract address to find smart money`
            , {
              "reply_markup": {
                "force_reply": true
              }
          ...
          this.bot.sendMessage(query.message.chat.id, `You are not an administrator that cannot be set up`)
        }
        break
      case "set_prc":
        if (query.from.username == this.params.adminName) {
        ...
        break
      case "Ethereum":
        this.bot.sendMessage(query.message.chat.id, `please enterEthereum prcLink replacement`
          , {
            "reply_markup": {
              "force_reply": true
            }
        ...
        break
      case "Arbitrum":
        this.bot.sendMessage(query.message.chat.id, `please enterArbitrum prcLink replacement`
          , {
            "reply_markup": {
              "force_reply": true
            }
        ...
        break
      case "Goerli":
        this.bot.sendMessage(query.message.chat.id, `please enterGoerli prcLink replacement`
          , {
            "reply_markup": {
              "force_reply": true
            }
      ...
      //Import wallet
      case "import_wallet":
        this.bot.sendMessage(query.message.chat.id, `Please enter the private key binding`
          , {
            "reply_markup": {
              "force_reply": true
            }
      ...
      //Add the opening
      case "add_rush":
        this.bot.sendMessage(query.message.chat.id, `Please enter the contract address`
          , {
            "reply_markup": {
              "force_reply": true
            }
      ...
      case "add_watch":
        this.bot.sendMessage(query.message.chat.id, `Please enter the address you need to listen to，Please use multiple,Separate examples0x,0x`
          , {
            "reply_markup": {
              "force_reply": true
            }
    ...
    if (pickerTaskWallet) {
      ...
      if (task) {
        ...
        this.bot.sendMessage(query.message.chat.id, `Failure to query the task`)
        return
      }
    }
    ...
    if (followBuyMatch) {
      ...
      if (find) {
        if (!find.follow_private_key) {
          this.bot.sendMessage(query.message.chat.id, `Please choose the wallet first`)
          return
        }
    ...
    if (followGasMatch) {
      ...
      this.bot.sendMessage(query.message.chat.id, `please entergastip`
        , {
          "reply_markup": {
            "force_reply": true
          }
    ...
    if (followSwapFeeMatch) {
      ...
      this.bot.sendMessage(query.message.chat.id, `Please enter the sliding point`
        , {
          "reply_markup": {
            "force_reply": true
          }
    ...
    if (followBuyAmountMatch) {
      ...
      this.bot.sendMessage(query.message.chat.id, `Please enter the purchase amountETHUnit`
        , {
          "reply_markup": {
            "force_reply": true
          }
    ...
    if (sell_percentMatch) {
      ...
      this.bot.sendMessage(query.message.chat.id, `Please enter the selling ratio`
        , {
          "reply_markup": {
            "force_reply": true
          }
    ...
    if (taskGasMatch) {
      ...
      this.bot.sendMessage(query.message.chat.id, `please entergas`
        , {
          "reply_markup": {
            "force_reply": true
          }
    ...
    if (taskStartTimeMatch) {
      ...
      this.bot.sendMessage(query.message.chat.id, `Please enter the start time format 2023/09/13T09:37:00`
        , {
          "reply_markup": {
            "force_reply": true
          }
    ...
    if (bindWatchNameMatch) {
      this.bot.sendMessage(query.message.chat.id, `👇👇👇Remark name for the following address，Please enter the name you need to note\n${bindWatchNameMatch[1]}`
        , {
          "reply_markup": {
            "force_reply": true
          }
  ...
  async handleManualSell(msg: any, chainId: number, address: string, percent: number, swapFee: number, gasFee: number, type: number, logId?: number) {
    ...
    if (logId) {
      ...
      if (wallet) {
        ...
        this.bot.sendMessage(msg.message.chat.id, 'The wallet of the order has been deleted')
        return
      }
    } else {
      let user = await this.getUserSetting(msg.from.id)
    ...
    if (fastPrice && fastPrice.price) {
      ...
      errorTamplate(this.bot, msg, contract, Number((Number(amountIn) / (10 ** Number(contract.decimals))).toFixed(3)), "", type, "Obtaining currency price failed")
    }
  }
  //Sell
  ...
  async handleManualBuy(msg: any, chainId: number, address: string, amount: number, swapFee: number, gasFee: number, type: number, logId?: number) {
    ...
    if (fastPrice && fastPrice.price) {
      ...
      errorTamplate(this.bot, msg, contract, amount, "", type, "Obtaining currency price failed")
    }
  }
  //Follow a single buy
