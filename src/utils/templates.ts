...
export const editorBuySuccessTemplate = async (bot: any, contract: any, user: any, currentGasPrice: number, wethPrice: number, log: any) => {
  ...
  let str = `🎰 <b>${contract.name}(<a href='${scan}'>$${contract.symbol}</a>) # ${chainEnum[contract.chain_id]}</b>\n\n` +
    ...
    `<b>🎀 pondETH: ${Number((poolEthBalance / (10 ** 18)).toFixed(5))} ETH</b>\n` +
    `<b>💎 pond${contract.symbol}: ${Number((poolTokenBalance / (10 ** Number(contract.decimals))).toFixed(5))} ${contract.symbol}</b>\n` +
    `<b>💰 Market value: $ ${formatUSDPrice(Number((total * Number(price)).toFixed(3)))}</b>\n\n` +
    `<b>📌 Buy address:</b>\n` +
    `<b>${log.address}</b>\n` +
...
export const pendingTamplate = async (bot: any, msg: any, contract: any, amount: number, hash: string, type: number) => {
  ...
  if (type == 1) {
    ...
    amountIn = `<b>💵 Follow a single buy：${BigNumber(amount).toFixed()} ETH</b>\n`
  } else if (type == 4) {
    amountIn = `<b>💵 Selling：${BigNumber(amount).toFixed()} ${contract.symbol}</b>\n`
  } else if (type == 5) {
    amountIn = `<b>💵 Grabbing：${BigNumber(amount).toFixed()} ${contract.symbol}</b>\n`
  }
  let str = `🎰 <b>${contract.name}(<a href='${scan}'>$${contract.symbol}</a>) # ${chainEnum[contract.chain_id]}</b>\n\n` +
    amountIn +
...
export const editorContractTemplate = async (bot: any, contract: any, user: any, currentGasPrice: string, wethPrice: number) => {
  ...
  let str = `🎰 <b>${contract.name}(<a href='${scan}'>$${contract.symbol}</a>) # ${chainEnum[contract.chain_id]}</b>\n\n` +
    ...
    `<b>💰 Market value: $ ${formatUSDPrice(Number((total * Number(price)).toFixed(3)))}</b>\n\n` +
    `<b>📌 default address:</b>\n` +
    `<b>${user.default_address ? user.default_address : 'There is no setting address'}</b>\n` +
    `<b>🎉 Account${contract.symbol}: ${userCoinBalance} ${contract.symbol}</b>\n` +
    `<b>💰 value: ${exchangeValue} ETH</b>\n` +
    `<b>📫 AccountETH: ${userEthBalance} ETH</b>\n`
  const contractKeyboard = [
    [
      {
        text: `💳 (${user.default_address ? user.default_address.substring(user.default_address.length - 15, user.default_address.length) : 'Click to select wallet'})`,
...
export const editorRushDetailTemplate = async (bot: any, query: any, contract: any, task: any, currentGasPrice: string, wethPrice: number) => {
  ...
  const contractKeyboard = [
    [
      {
        text: `💳 (${task.address ? task.address.substring(task.address.length - 15, task.address.length) : 'Click to select wallet'})`,
...
export const addWalletTemplate = (bot: any, msg: any) => {
  let str = "❗️❗️❗️ <b>Please recognize the DogBOT，Do not send it to the scammer robot</b>\n\n<b>Please lose the way you choose to bind a wallet</b>"
  bot.editMessageText(str, {
    chat_id: msg.message.chat.id,
    message_id: msg.message.message_id,
    parse_mode: "HTML",
...
export const rushTemplate = async (bot: any, msg: any, db: any) => {
  ...
  let str = "❗️❗️❗️ <b>Dogs are risky，Investment needs to be cautious</b>\n\n"
  let listKeyboard = []
...
export const handleWatchTemplate = async (bot: any, msg: any, address: string, db: any) => {
  ...
  if (find) {
    ...
    const followKeyboard = [
      [
        {
          text: `💳 (${account ? account.address.substring(account.address.length - 15, account.address.length) : 'Click to select wallet'})`,
...
export const watchTemplate = async (bot: any, msg: any, db: any) => {
  ...
  if (watchList.length) {
    ...
    str += `*Co -prisoner listening(${watchList.length})Address，Remaining can be added(${10 - watchList.length})Address*\n\n`
    watchList.forEach((item, index) => {
      let isFollow = (item.follow_buy == 1 || item.follow_sell == 1) ? "（In order）\n" : ""
      str += "wallet(" + (index + 1) + ")\n`" + item.address + "`\n" + isFollow + "\n"
