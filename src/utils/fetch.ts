...
//Batch acquisition transfer event
export const batchGetTransferEvent = (
  address: string,
  chainId: number,
  startBlock: number,
...
export const getFirstPoolForBlockNumber = (contract: insertItem) => {
  return new Promise<{
    ...
    let [token0, token1] = tokenList[0].sortsBefore(tokenList[1])
      ...
      if (contract.chain_id == 42161) {
        let camelot = await web3.eth.getPastLogs({
          ...
          topics: [
            ...
            web3.eth.abi.encodeParameter("address", token0.address),
            web3.eth.abi.encodeParameter("address", token1.address),
          ],
        });
    ...
    if (events.length) {
      ...
      if (firstPool.address == config.camelotFactory) {
        let decode = web3.eth.abi.decodeParameters(
          ["address", "uint256"],
          firstPool.data
        );
        resolve({
...
//Get the first transaction blockNumber
export const getFirstTransactionBlockNumber = (
  address: string,
  startNumber: number,
  chainId: number,
  count: number = 2000
    ...
    web3.eth
      ...
      .then(async (res) => {
        if (res.length > 1) {
          ...
          let blockNumber = await getFirstTransactionBlockNumber(
            address,
            startNumber,
            chainId,
            count + 2000
...
//Treatment to get the first price to get the first pond
  ...
  return new Promise<{
      ...
      try {
        if (firstPool.version == "camelotv2") {
          let list = await web3.eth.getPastLogs({
            ...
            toBlock:
              currentBlockNumber - firstPool.blockNumber > 10000
                ? firstPool.blockNumber + 10000
                : currentBlockNumber,
          ...
          if (list.length) {
            ...
            list.forEach((item) => {
              item.decode = web3.eth.abi.decodeParameters(
                ["uint256", "uint256"],
                item.data
              );
            });
            for (let event of list) {
              let amount0 = 0;
              let amount1 = 0;
              //Detect whether it is a unilateral pool. If it is so accumulated to the next pool
          ...
          if (list.length) {
            ...
            for (let event of list) {
              ...
              if (Number(event.decode[2]) > 0 && Number(event.decode[3]) > 0) {
                let params = {
                  amount0: BigNumber(
                    Number(event.decode[2]) + amount0
                  ).toFixed(),
                  amount1: BigNumber(
                    Number(event.decode[3]) + amount1
                  ).toFixed(),
                  token0: firstPool.token0,
                  token1: firstPool.token1,
                  blockNumber: event.blockNumber,
    ...
    if (events.length) {
      ...
      for (let item of events) {
        //Determine whether it is a unilateral pool ，If it is a unilateral pool, then the next one
        if (Number(item.amount0) > 0 && Number(item.amount1) > 0) {
          //If it is a god sword 、sushi、v2 Use all v2Obtain
          ...
          poolBalance =
            firstPool.token0 == Config[contract.chain_id].stableContract[0]
              ? firstPool.reserve0
              : firstPool.reserve1;
          let price = await computedV2Price(
            item.token0,
            item.token1,
            firstPool,
            contract.chain_id
    ...
    resolve({
      price: BigNumber(firstPrice).toFixed(),
      poolBalance: BigNumber(
        (Number(poolBalance) / 10 ** 18).toFixed(5)
      ).toFixed(),
      firstBlockNumber,
    });
  });
};
//Get block information in batches
export const batchBlockRequest = (
  startNumber: number,
  endNumber: number,
  chainId: number
) => {
        ...
        res.data.forEach((item) => {
          if (item.result && item.result.transactions.length) {
            item.result.transactions.forEach((transaction) => {
              transaction.blockNumber = BigNumber(
                Number(ethBigNumber.from(transaction.blockNumber).toBigInt())
              ).toFixed();
              transaction.gas = BigNumber(
                Number(ethBigNumber.from(transaction.gas).toBigInt())
              ).toFixed();
              transaction.gasPrice = BigNumber(
                Number(ethBigNumber.from(transaction.gasPrice).toBigInt())
              ).toFixed();
              transaction.value = BigNumber(
                Number(ethBigNumber.from(transaction.value).toBigInt())
              ).toFixed();
              transaction.transactionIndex = BigNumber(
                Number(
                  ethBigNumber.from(transaction.transactionIndex).toBigInt()
                )
              ).toFixed();
              transaction.nonce = BigNumber(
                Number(ethBigNumber.from(transaction.nonce).toBigInt())
              ).toFixed();
            });
...
//Details of transaction in batches
export const batchGetTransaction = (hashs: string[], chainId: number) => {
  return new Promise<any[]>((resolve) => {
    if (hashs.length) {
      ...
      hashs.forEach((item) => {
        batch.add(
          web3.eth.getTransaction.request(item, (err, res) => {
            success++;
            if (res) {
...
//Details of transaction in batches
export const batchGetTransactionReceipt = (
  hashs: string[],
  chainId: number
) => {
  return new Promise<any[]>((resolve) => {
    if (hashs.length) {
      ...
      hashs.forEach((item) => {
        batch.add(
          web3.eth.getTransactionReceipt.request(item, (err, res) => {
            success++;
            if (res) {
              transactions.push(res);
            }
...
export const batchGetTransactionCount = (addresses: any[], chainId: number) => {
  return new Promise<baseData[]>((resolve) => {
    ...
    addresses.forEach((item) => {
      batch.add(
        web3.eth.getTransactionCount.request(item.address, (err, res) => {
          success++;
          if (res) {
            if (res < 3000) {
              result.push({
...
//Batch acquisition erc20basic information
...
//Get a single basic information
...
export const batchGetPair = (addresses: string[], chainId: number) => {
  return new Promise<v2Pool[]>(async (resolve) => {
    try {
      ...
      let result = [];
      let contract = Config[chainId].DANDAOFactoryContract
        ? Config[chainId].DANDAOFactoryContract
        : getDanDaoContract(chainId);
      for (let item of chunkAddresses) {
...
//Batch acquisition erc20Balance
export const batchCheckERC20Balance = (
  checkERC20Items: checkERC20Item[],
  chainId: number
) => {
  return new Promise<checkERC20Response[]>(async (resolve) => {
    try {
      ...
      let contract;
      if (chainId == 1) {
        contract = Config[chainId].DANDAONFTFactoryContract
          ? Config[chainId].DANDAONFTFactoryContract
          : getDanDaoNFTContract(chainId);
      } else {
        contract = Config[chainId].DANDAOFactoryContract
          ? Config[chainId].DANDAOFactoryContract
          : getDanDaoNFTContract(chainId);
      }
      for (let item of chunkCheckERC20Items) {
        let contractResult = await contract.methods
          .batchCheckERC20Balance(item)
          .call();
        result = result.concat(contractResult);
      }
      resolve(result);
...
//Get the address in batches ethBalance
...
//Batch advance execution acquisition gas
export const telegramBatchEstimateGas = (
  tasks: telegramTask[],
  chainId: number
) => {
  return new Promise<any[]>((resolve) => {
    ...
    tasks.forEach((task) => {
      ...
      batch.add(
        web3.eth.estimateGas.request(
          {
            from: account.address,
            to: config.DANDAOTrade,
            data: task.encode_data,
            value: BigNumber(
              Number(Number(task.amount) * 10 ** 18) + config.DANDAOFee
            ).toFixed(),
          },
          (err, res) => {
            success++;
...
export const estimateGas = (params: any, web3: Web3) => {
  return new Promise<{ success: boolean; gas: number; msg: string }>(
    (resolve) => {
      web3.eth
        .estimateGas(params)
        .then((gas) => {
...
//ask dextoolbasic information
export const getDEXInfo = (address: string, chainId: number) => {
  return new Promise((resolve) => {
    axios({
      url: `https://www.dextools.io/shared/data/pair?address=${address.toLocaleLowerCase()}&chain=${
        Config[chainId].name
      }&audit=true&locks=true`,
      method: "get",
    })
      .then((res) => {
        if (res.data.data[0]) {
...
export const getPendingEvent = (chainId: number, blockNumber) => {
  return new Promise<any>((resolve) => {
    const web3: Web3 = getProvider(chainId);
    web3.eth
      .getPastLogs({ fromBlock: blockNumber, toBlock: blockNumber })
      .then((events) => {
        resolve(events);
      })
      .catch((err) => {
...
//Obtain pendingblockinformation
export const getPendingBlock = (chainId: number) => {
  return new Promise<any>((resolve) => {
    const web3: Web3 = getProvider(chainId);
    web3.eth
      .getBlock("pending", true)
      .then((block) => {
        resolve(block);
      })
      .catch((err) => {
...
//View authorization
export const checkApproveBalance = (
  user: string,
  target: string,
  spender: string,
  chainId: number
  ...
  return new Promise(async (resolve) => {
    try {
      ...
      let approveBalance = await contract.methods
...
export const batchSendTransaction = (
  ...
  return new Promise<boolean>((resolve) => {
    try {
      ...
      rawTransactions.forEach((item) => {
        batch.add(
          web3.eth.sendSignedTransaction.request(item, (err, res) => {
            success++;
...
//Whether the address of the batch detection is a contract
export const batchGetCode = (addresses: string[], chainId: number) => {
  return new Promise<any[]>((resolve) => {
    try {
      ...
      addresses.forEach((item) => {
        batch.add(
          web3.eth.getCode.request(item, (err, res) => {
            success++;
            if (res == "0x") {
              result.push(item);
            }
...
//Batch acquisition nftAll holder address
export const getNFTAllAddress = (
  address: string,
  startToken: number,
  endToken: number
) => {
  return new Promise((resolve) => {
    ...
    conctract.methods
      .getNftAllAddress(address, startToken, endToken)
      .call()
      .then((result) => {
        resolve(result);
      })
  ...
  return new Promise((resolve) => {
    ...
    for (let item of newList) {
      ...
      factoryContract.methods
        .batchQueryDNSName("0x74e20bd2a1fe0cdbe45b9a1d89cb7e0a45b36376", notes)
        .call()
        .then((result) => {
...
//Get all the transfer records of the contract
export const getContractAllTransferEvent = (
  address: string,
  startBlock: number,
  chainId: number
) => {
    ...
    for (let item of sliceAddresses) {
      try {
        let result = await axios({
          url: `https://api.etherscan.io/api?module=contract&action=getcontractcreation&contractaddresses=${item.join(
            ","
          )}&apikey=XJZ14US5XTCG3JR1V716UBJP7XQ2IVNZ3S`,
          method: "get",
        });
